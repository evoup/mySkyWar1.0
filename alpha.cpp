			 ...
			 int iDestX, int iDestY, LPRECT lprcSource, DWORD dwMode )
{
		...
		do
		{
			...
			while ( i-- > 0 )
			{
				...
				if ( dwSourceTemp != 0 )
				{
					...
					{
						// ... make sure the first target pixel won磘 change.
					...
					{
						// ... make sure the second target pixel won磘 change.
	...
	case RGBMODE_16:
		...
		// Create the bit mask used to clear the lowest bit of each color channel磗 mask.
...
not_black555:
				...
				/* Note: Two lines together are assumed to pair 
				         in the processor磗 U- and V-pipes. */
...
skip16:
	...
	case RGBMODE_24:
		...
		// Create a mask that lets us clear each channel磗 lowest bit.
...
do_blend24:
			...
			pand		mm0, mm1				// Clear each target channel磗 lowest bit.

			pand		mm1, [esi]				// Clear each source channel磗 lowest bit.
...
next24:
	...
	case RGBMODE_32:
		...
		// Create a mask that allows us to clear each channel磗 highest bit.
...
not_black32:
				...
				pand		mm0, mm3				// Clear each source channel磗 highest bit.
				psrld		mm1, 1					// Divide the source by 2.

				pand		mm1, mm3				// Clear each target channel磗 highest bit.
